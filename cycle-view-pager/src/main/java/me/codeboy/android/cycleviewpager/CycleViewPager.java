...
public class CycleViewPager extends Fragment implements OnPageChangeListener, View.OnTouchListener {
    ...
    public void onPageScrolled(int position, float offset, int pixelOffset) {
        Log.i("cycleViewPager-info", "滚动中。position: " + position + ",offset: " + offset + ",pixelOffset: " + pixelOffset);
    }

    @Override
    public void onPageSelected(int arg0) {
        Log.i("cycleViewPager-info", "选定界面");
        int max = views.size() - 1;
        int position = arg0;
    ...
    private void setIndicator(int selectedPosition) {
        for (int i = 0; i < indicators.length; i++) {
            indicators[i].setBackgroundResource(R.drawable.cycleviewpager_indicator_circle_black);
        }
        if (indicators.length > selectedPosition) {
            indicators[selectedPosition].setBackgroundResource(R.drawable.cycleviewpager_indicator_circle_red);
        }
    }
